...
def analyse_transactions(state: AgentState) -> AgentState:
    ...
    consistency = min(len(monthly) / 6 * 100, 100)

    # Income/expense ratio (capped for scoring)
    ie_ratio = income / expense if expense > 0 else income

    # Order book strength: total capacity × active orders (normalised to 100)
    total_capacity = sum(a.get("capacity_metres_per_day", 0) for a in loom_assets)
    total_active = sum(a.get("active_orders", 0) for a in loom_assets)
    order_strength = min((total_capacity * total_active) / 500 * 100, 100)
...
def llm_reasoning(state: AgentState) -> AgentState:
    ...
    try:
        ...
        reasoning = "Automated evaluation based on transaction metrics."
...
def compute_final_score(state: AgentState) -> AgentState:
    ...
    Weights:
      transaction_consistency   30%
      income_to_expense_ratio   25%  (normalised 0–100)
      order_book_strength       20%
      experience_bonus          15%
...
async def run_credit_evaluation(payload: dict) -> dict:
    """
    Run the credit evaluation agent.
