...
export function InsuranceProvidersTab({ providers, onSelectProvider }: Props) {
        ...
        {groupProviders.map((provider) => (
          <motion.div key={provider.id} variants={staggerItem} className="h-full">
            <Card
              ...
              className="h-full border-2 border-slate-100 hover:border-slate-300 transition-all hover:shadow-md cursor-pointer flex flex-col justify-between"
            >
              <CardContent className="p-6 flex flex-col justify-between h-full">
                ...
                <Button
                  variant="outline"
                  fullWidth
                  className="mt-6 border-slate-200 text-slate-700 hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-colors shadow-sm"
                  onClick={(e) => {
                    e.stopPropagation();
