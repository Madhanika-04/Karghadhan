...
replaceInFile(insuranceProvidersTabPath, [
    { search: /src=\{provider\.logo\}/g, replace: 'src={provider.logo} /* Assume logos are locally imported in data */' }
]);
...
// 5. Global replacement for logoKargha
...
walkSync(path.join(__dirname, 'src'), function(filePath) {
    let content = fs.readFileSync(filePath, 'utf-8');
    let original = content;
    
    // Replace import logoKargha from '../../assets/logokargha.png' or '../assets/logokargha.png'
    content = content.replace(/import\s+logoKargha\s+from\s+['"](?:\.\.\/)+assets\/logokargha\.png['"];/g, "import logoKargha from '@/assets/logos/logoKargha.png';");
    
    // Some are just import logoKargha from '../assets/logokargha.png';
    content = content.replace(/import\s+logoKargha\s+from\s+['"](?:\.\/)+assets\/logokargha\.png['"];/g, "import logoKargha from '@/assets/logos/logoKargha.png';");

    if (content !== original) {
        fs.writeFileSync(filePath, content, 'utf-8');
        console.log(`Updated logoKargha import in ${filePath}`);
    }
});
